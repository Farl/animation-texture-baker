...
Shader "VertexAnimationTexture"
{
	Properties
	{
		[HideInInspector] _AlphaCutoff("Alpha Cutoff ", Range(0, 1)) = 0.5
		[HideInInspector] _EmissionColor("Emission Color", Color) = (1,1,1,1)
		[ASEBegin]_PosTex("Pos Tex", 2D) = "black" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float2 texCoord70 = v.texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				#ifdef _PARTICLE_ON
				float staticSwitch71 = texCoord70.y;
				#else
				...
				#else
				float staticSwitch40 = saturate( temp_output_19_0 );
				#endif
				float2 appendResult21 = (float2(( ( staticSwitch71 + 0.5 ) * _PosTex_TexelSize.x ) , staticSwitch40));
				float2 sampleUV41 = appendResult21;
				float4 posDiff35 = tex2Dlod( _PosTex, float4( sampleUV41, 0, 0.0) );
				...
				#endif
				float4 normalizeResult64 = ASESafeNormalize( ( staticSwitch45 + float4( v.ase_normal , 0.0 ) ) );
				...
				float3 vertexValue = staticSwitch39.rgb;
					...
					v.vertex.xyz += vertexValue;
				#endif
				v.ase_normal = normalizeResult64.rgb;
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v )
			{
				...
				float2 texCoord70 = v.ase_texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				#ifdef _PARTICLE_ON
				float staticSwitch71 = texCoord70.y;
				#else
				float staticSwitch71 = (float)v.ase_vertexID;
